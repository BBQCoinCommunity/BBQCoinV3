...
**Optional dependencies** :
```bash
# Qt (required for bbqcoin-qt GUI)
...
According to installed dependencies, the following steps will compile `bbqcoind`, `bbqcoin-cli` and `bbqcoin-qt`.
...
#### Enable Qt GUI
Create `bbqcoin-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
