...
---------|--------------------
Linux    | `$HOME/.bbqcoin/`
macOS    | `$HOME/Library/Application Support/BBQCoin/`
Windows  | `%APPDATA%\Dogecoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for the `bbqcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `bbqcoin.conf`       | User-defined configuration settings for `bbqcoind` or `bbqcoin-qt`; can be specified by `-conf` option
`./`               | `bbqcoind.pid`        | Stores the process ID (PID) of `bbqcoind` or `bbqcoin-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `bbqcoind` or `bbqcoin-qt`
...
`bbqcoin-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
